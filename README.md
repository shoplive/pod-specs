...
Shoplive iOS SDK 배포용 CocoaPods Spec 저장소입니다.
...
```ruby
source 'https://github.com/shoplive/pod-specs.git'  # Shoplive spec repo
...
| `Unable to find a specification for ShopliveSDK` | Podfile에 spec repo `source` 누락 | [2-1](#2-1-podfile-설정)의 source 확인 |
| Shoplive 외 다른 pod 설치 실패 | 공식 CDN `source` 누락 | [2-1](#2-1-podfile-설정)의 두 번째 source 추가 |
